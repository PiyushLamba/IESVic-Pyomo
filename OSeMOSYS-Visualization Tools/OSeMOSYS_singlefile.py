...
osmosis_df = pd.DataFrame(osmosis)
...
def ex_fig_general(plot_name, region):
    ...
    for y_title in y_tile_list:
        ...
        if len(y_data) > 0:
            ...
            if plot_name[:-1] in y_title:
                y_title = y_title.replace(plot_name[:-1],'')
...
#    position += 1;
    colours = ['black', 'slategrey', 'salmon', 'red', 'mistyrose', 'orange', 'tan', 'darkgreen', 'turquoise', 'saddlebrown', 'yellowgreen', 'gold']
#    for i in range(0,25):
#        r = random.random()
#        g = random.random()
#        b = random.random()
...
#        colours.append(rgb)
    ...
    if plot_name == "New Capacity*":
        for i in range(len(all_y_data)):
            ax.bar(YEAR_LIST, all_y_data[i], label=new_y_title_list[i], color=colours[i])
    elif plot_name == "Annual Emissions*":
        if "CH4" in new_y_title_list[0]:
            ax.set_ylabel("Annual CH4 Emissions(Megatonnes/GWh)")
            ax.stackplot(YEAR_LIST, *all_y_data, labels=new_y_title_list, colors='skyblue')
        elif "CO2" in new_y_title_list[0]:
            ax.set_ylabel("Annual CO2 Emissions(Megatonnes/GWh)")
            ax.stackplot(YEAR_LIST, *all_y_data, labels=new_y_title_list, colors='purple')
        fig.legend(loc = 'lower center', fontsize = '13')
    else:
        ax.stackplot(YEAR_LIST, *all_y_data, colors=colours)
...
#    ax.legend(loc='upper left', fontsize = '10')
    ...
    if plot_name == "Total Capacity*":
        ...
        fig.legend(loc = 'lower center', fontsize = '11', ncol = 12)
