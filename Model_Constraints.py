...
def S1_RateofStorageCharge_rule(model, r, s, ls, ld, lh, y):
    result1 = sum(model.RateOfActivity[r,l,t,m,y] * model.TechnologyStorage[r,t,s,m] * 
    model.Conversionls[l,ls] * model.Conversionld[l,ld] * model.Conversionlh[l,lh]
    for m in model.MODE_OF_OPERATION for l in model.TIMESLICE for t in model.TECHNOLOGY
    if model.TechnologyStorage[r,t,s,m] > 0)
    return result1 == model.RateOfStorageCharge[r,s,ls,ld,lh,y]

def S2_RateofStorageDischarge_rule(model, r, s, ls, ld, lh, y):
    result1 = sum(model.RateOfActivity[r,l,t,m,y] * model.TechnologyStorage[r,t,s,m] * 
    model.Conversionls[l,ls] * model.Conversionld[l,ld] * model.Conversionlh[l,lh]
    for m in model.MODE_OF_OPERATION for l in model.TIMESLICE for t in model.TECHNOLOGY
    if model.TechnologyStorage[r,t,s,m] > 0)
    return result1 == model.RateOfStorageDischarge[r,s,ls,ld,lh,y]

def S3_NetChargeWithinYear_rule(model, r, s, ls, ld, lh, y):
    result1 = sum((model.RateOfStorageCharge[r,s,ls,ld,lh,y] - model.RateOfStorageDischarge[r,s,ls,ld,lh,y]) *
    model.YearSplit[l,y] * model.Conversionls[l,ls] * model.Conversionld[l,ld] * model.Conversionlh[l,lh]
    for l in model.TIMESLICE if model.Conversionls[l,ls] > 0 and model.Conversionld[l,ld] > 0 
    and model.Conversionlh[l,lh] > 0)
    return result1 == model.NetChargeWithinYear[r,s,ls,ld,lh,y]
...
def S4_NetChargeWithinDay_rule(model, r, s, ls, ld, lh, y):
    ...
    return result1 == model.NetChargeWithinDay[r,s,ls,ld,lh,y]

def S5_S6_StorageLevelYearStart_rule(model, r, s, y):
    if y == min(model.YEAR):
        return model.StorageLevelYearStart[r,s, y] == model.StorageLevelYearStart[r,s, y]
    else:
        result1 = model.StorageLevelYearStart[r,s, y - 1] + sum(
            model.NetChargeWithinYear[r,s,ls,ld,lh,y - 1] 
        for ls in model.SEASON for ld in model.DAYTYPE for lh in model.DAILYTIMEBRACKET)
        return result1 ==  model.StorageLevelYearStart[r,s, y]

def S7_S8_StorageLevelYearFinish_rule(model, r, s, y):
    if y < max(model.YEAR):
        return model.StorageLevelYearStart[r,s, y+1] == model.StorageLevelYearStart[r,s, y+1]
    else:
        result1 = model.StorageLevelYearStart[r,s, y] + sum(
            model.NetChargeWithinYear[r,s,ls,ld,lh,y] 
        for ls in model.SEASON for ld in model.DAYTYPE for lh in model.DAILYTIMEBRACKET)
        return result1 ==  model.StorageLevelYearFinish[r,s, y]
